class NanakoMicGateProcessor extends AudioWorkletProcessor {
  constructor(){
    super();this.noise=.006;this.open=false;this.hold=0;this.buffer=[];this.pre=[];
    this.port.onmessage=e=>{if(e.data?.type==="reset"){this.noise=.006;this.open=false;this.hold=0;this.buffer=[];this.pre=[];}};
  }
  ...
  process(inputs,outputs){
    ...
    const rms=Math.sqrt(sum/block.length),threshold=Math.max(.011,this.noise*2.8);
    if(!this.open&&rms<threshold)this.noise=this.noise*.97+rms*.03;
    if(rms>=threshold){if(!this.open){this.open=true;for(const old of this.pre)this.emit(old);this.pre=[];}this.hold=Math.round(sampleRate*1.1/2048);}
    else if(this.open&&this.hold>0)this.hold--;else if(this.open){this.open=false;this.port.postMessage({type:"gate_closed"});}
    if(this.open)this.emit(block);else{this.pre.push(block);while(this.pre.length>Math.ceil(sampleRate*.4/2048))this.pre.shift();}
    return true;
  }
}
