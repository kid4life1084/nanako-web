...
window.nanakoStartupProgress=(percent,label="Loading Nanako…",ready=false)=>{
  const box=document.getElementById("startupProgress");
  const fill=document.getElementById("startupProgressFill");
  const text=document.getElementById("startupProgressText");
  const value=document.getElementById("startupProgressPercent");
...
async function ensureCurrentServiceWorker(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("./sw.js?v=2.6.9",{scope:"./",updateViaCache:"none"});
    await reg.update();
...
function deviceIsActuallyLandscape(){
  // Step 2.27.0: NEVER infer device orientation from visualViewport dimensions.
...
function syncBodyMotionForFrame(frame){
  ...
  const clipName=name==="neutral"?"standing":name;
...
async function loadOptionalBodyAnimations(){
  ...
  const result=await renderer.loadBodyAnimations({
    angry:"./static/animations/nanako_angry.fbx?v=2.6.3",
    thinking:"./static/animations/nanako_thinking.fbx?v=2.6.3",
    clapping:"./static/animations/nanako_clapping.fbx?v=2.6.3"
  });
...
async function loadProductionBodyAnimations(){
  ...
  bodyAnimationsLoading=(async()=>{
    const result=await window.nanako3DRenderer.loadBodyAnimations({
      standing:"./static/animations/nanako_idle_replacement.fbx?v=2.6.3"
    });
...
window.addEventListener("nanako3d-ready",async()=>{
  ...
  window.nanakoStartupProgress?.(94,"Preparing Nanako…");
  ...
  if(ready){
    ...
    window.nanakoStartupProgress?.(100,"Ready",true);
  }else{
    window.nanakoStartupProgress?.(94,"Animation loading issue");
  }
});
...
let animationSfxCtx=null,animationSfxMasterGain=null;
let animationSfxSources=new Set(),animationSfxGeneration=0;
const animationSfxBuffers=new Map(),animationSfxPromises=new Map();
const ANIMATION_SFX_VOLUME=0.70;

function animationSfxContext(){
  if(animationSfxCtx&&animationSfxCtx.state!=="closed")return animationSfxCtx;
  const AC=window.AudioContext||window.webkitAudioContext;
  if(!AC)return null;
  animationSfxCtx=new AC();
  animationSfxMasterGain=animationSfxCtx.createGain();
  animationSfxMasterGain.gain.value=ANIMATION_SFX_VOLUME;
  animationSfxMasterGain.connect(animationSfxCtx.destination);
  return animationSfxCtx;
}

async function loadAnimationSfxBuffer(src){
  const key=String(src||"");
  if(!key)return null;
  if(animationSfxBuffers.has(key))return animationSfxBuffers.get(key);
  if(animationSfxPromises.has(key))return animationSfxPromises.get(key);
  const promise=(async()=>{
    const ac=animationSfxContext();if(!ac)return null;
    ...
    animationSfxBuffers.set(key,decoded);
    animationSfxPromises.delete(key);
    return decoded;
  })().catch(err=>{animationSfxPromises.delete(key);console.warn("[Nanako SFX] preload failed",key,err);return null});
  animationSfxPromises.set(key,promise);
  return promise;
}

function preloadAnimationSfx(){
  void loadAnimationSfxBuffer("./static/sfx/wink.wav?v=2.6.3");
  void loadAnimationSfxBuffer("./static/sfx/single_clap.wav?v=2.6.3");
  void loadAnimationSfxBuffer("./static/sfx/level_up.mp3?v=2.6.3");
  void loadAnimationSfxBuffer("./static/sfx/daily_challenge_pass.wav?v=2.6.3");
}

async function playLevelUpSfx(){
  const ac=animationSfxContext();if(!ac)return false;const buffer=await loadAnimationSfxBuffer("./static/sfx/level_up.mp3?v=2.6.3");if(!buffer)return false;try{if(ac.state!=="running")await ac.resume()}catch{}
...
function levelUpClapPlan(){return {source:"nanako-learning-reward",renderer_contract:"nanako-vrm-1.1-python-plan-fbx",renderer_mode:"vrm-3d-only",kind:"level-up-clapping",emotion:"happy",body_motion:"clapping",duration_ms:2600,sample_ms:100,sfx:{src:"./static/sfx/single_clap.wav?v=2.6.3",hits_ms:[140,432,724,1016,1308,1600,1892,2184,2476],hit_volumes:[1.00,0.60,0.82,0.48,0.74,0.56,0.90,0.64,0.78],stop_ms:2600,name:"single_clap_levelup",volume:1},frames:[{t:0,emotion:"happy",action:"clapping",eyes:"open",mouth:"closed",body_motion:"clapping",scale:1,translate_y:0},{t:2500,emotion:"happy",action:"clapping",eyes:"open",mouth:"closed",body_motion:"clapping",scale:1,translate_y:0},{t:2600,emotion:"happy",action:"idle",eyes:"open",mouth:"closed",body_motion:"neutral",scale:1,translate_y:0}]};}
async function celebrateLearningLevelUp(event){
  ...
  else await new Promise(resolve=>{playAnimationPlan(levelUpClapPlan(),{onComplete:resolve});});
  status("Nanako is continuing...");
}
let dailyChallengeNoticeQueue=[],dailyChallengeNoticeBusy=false;
async function playDailyChallengePassSfx(){
  const ac=animationSfxContext();if(!ac)return false;const buffer=await loadAnimationSfxBuffer("./static/sfx/daily_challenge_pass.wav?v=2.6.3");if(!buffer)return false;
...
function completedMissionIds(profile){return new Set((Array.isArray(profile?.active_missions)?profile.active_missions:[]).filter(m=>m?.completed).map(m=>Number(m.id)).filter(Number.isFinite))}
function missionById(profile,id){return (Array.isArray(profile?.active_missions)?profile.active_missions:[]).find(m=>Number(m?.id)===Number(id))||null}
function enqueueDailyChallengeNotice(text,{sfx="pass"}={}){
  dailyChallengeNoticeQueue.push({text:String(text||""),sfx:String(sfx||"")});void pumpDailyChallengeNotices();
}
async function pumpDailyChallengeNotices(){
  if(dailyChallengeNoticeBusy)return;dailyChallengeNoticeBusy=true;
  const box=document.getElementById("dailyChallengePassNotice"),copy=document.getElementById("dailyChallengePassText");
  try{while(dailyChallengeNoticeQueue.length){const item=dailyChallengeNoticeQueue.shift();if(!box||!copy)break;copy.textContent=item.text;box.hidden=false;box.classList.remove("show");void box.offsetWidth;box.classList.add("show");if(item.sfx==="level_up")void playLevelUpSfx();else if(item.sfx==="pass")void playDailyChallengePassSfx();await new Promise(r=>setTimeout(r,4000));box.classList.remove("show");await new Promise(r=>setTimeout(r,180));box.hidden=true;}}finally{dailyChallengeNoticeBusy=false}
}
function announceNewChallengeCompletions(beforeIds,afterProfile){
  const after=completedMissionIds(afterProfile),fresh=[...after].filter(id=>!beforeIds.has(id));
  for(const id of fresh){const m=missionById(afterProfile,id);if(m)enqueueDailyChallengeNotice(m.text,{sfx:"pass"});}
  const beforeAll=beforeIds.size>=5,afterAll=(Array.isArray(afterProfile?.active_missions)&&afterProfile.active_missions.length===5&&afterProfile.active_missions.every(m=>m?.completed));
  if(afterAll&&!beforeAll)enqueueDailyChallengeNotice("Daily Challenge Complete!",{sfx:"level_up"});
}


function stopAnimationSfx(){
  animationSfxGeneration+=1;
  for(const source of [...animationSfxSources]){
    try{source.stop()}catch{}
    try{source.disconnect()}catch{}
  }
  animationSfxSources.clear();
}

function scheduleAnimationSfx(plan){
  const cue=plan?.sfx;if(!cue?.src)return Promise.resolve(null);
  const syncWink=/wink/i.test(String(cue.name||""));
  const generation=animationSfxGeneration;
  const starts=Array.isArray(cue.hits_ms)&&cue.hits_ms.length
    ? cue.hits_ms.map(v=>Math.max(0,Number(v)||0))
    : [Math.max(0,Number(cue.start_ms)||0)];
  const planStop=Math.max(0,Number(cue.stop_ms)||Number(plan?.duration_ms)||0);
  const requestedAt=performance.now();
  return (async()=>{
    const buffer=await loadAnimationSfxBuffer(String(cue.src));
    if(!buffer||generation!==animationSfxGeneration)return null;
    const ac=animationSfxContext();if(!ac)return;
    try{if(ac.state!=="running")await ac.resume()}catch{}
    if(generation!==animationSfxGeneration)return null;
    if(syncWink&&ac.state!=="running")return null;
    const hitVolumes=Array.isArray(cue.hit_volumes)?cue.hit_volumes:[];
    let syncStartAt=null;
    for(let hitIndex=0;hitIndex<starts.length;hitIndex++){
      const startMs=starts[hitIndex];
      const lateness=performance.now()-(requestedAt+startMs);
      // Missing an SFX hit is preferable to playing it a second late.
      if(lateness>80&&!syncWink)continue;
      const source=ac.createBufferSource();
      source.buffer=buffer;
      const hitGain=ac.createGain();
      const perHitVolume=hitVolumes.length?Number(hitVolumes[hitIndex]??cue.volume??1):Number(cue.volume??1);
      hitGain.gain.value=Math.max(0,Math.min(1,perHitVolume));
      source.connect(hitGain);hitGain.connect(animationSfxMasterGain);
      animationSfxSources.add(source);
      source.onended=()=>{animationSfxSources.delete(source);try{source.disconnect()}catch{}try{hitGain.disconnect()}catch{}};
      const when=syncWink
        ? ac.currentTime+.02+Math.max(0,(requestedAt+startMs-performance.now())/1000)
        : ac.currentTime+Math.max(0,(requestedAt+startMs-performance.now())/1000);
      source.start(when);
      if(syncWink)syncStartAt=performance.now()+Math.max(0,(when-ac.currentTime)*1000);
      if(planStop>startMs){
        const stopWhen=syncWink?when+(planStop-startMs)/1000:ac.currentTime+Math.max(0,(requestedAt+planStop-performance.now())/1000);
        try{source.stop(stopWhen)}catch{}
      }
    }
    console.log(`[Nanako SFX] ${cue.name||"animation"} hits=${starts.join(",")}ms stop=${planStop}ms volume=${Math.round(ANIMATION_SFX_VOLUME*100)}%`);
    return syncStartAt;
  })();
}
preloadAnimationSfx();
...
function playAnimationPlan(plan,{loop=false,onComplete=null,mediaClock=null}={}){
  ...
  if(plan.kind==="post-speech-wink"&&!plan.sfx){
    plan={...plan,duration_ms:Math.max(900,Number(plan.duration_ms)||0),sfx:{src:"./static/sfx/wink.wav?v=2.6.3",start_ms:400,stop_ms:1400,name:"wink-post-speech",volume:1}};
  }
  ...
  const tick=now=>{
    ...
    if(mediaClock?mediaStillPlaying:(loop||elapsed<duration)){
      ...
      animationRaf=0;
      if(plan.kind!=="post-speech-wink")stopAnimationSfx();
...
function finishPlanWithPostHold(plan,{idleEmotion=null,onDone=null}={}){
  ...
  if(postPlan?.frames?.length){
    playAnimationPlan(postPlan,{onComplete:()=>{
      const audioTailMs=postPlan.kind==="post-speech-wink"&&postPlan.sfx
        ? Math.max(0,Number(postPlan.sfx.stop_ms||0)-Number(postPlan.duration_ms||0)) : 0;
      setTimeout(()=>{
        returnToPythonIdle({emotion:target,bodyMotion:idlePlanBodyMotion(postPlan)||targetBodyMotion});
        if(typeof onDone==="function")onDone();
      },Math.min(1000,audioTailMs));
    }});
    return;
  }
  if(hold>0&&post){
...
const CLIENT_BUILD="12.0.139",RELEASE_VERSION="2.6.9",API="https://nanako-web-pokbkohedy.ap-southeast-1.fcapp.run",CHAT=`${API}/api/chat`,VISION_IDENTIFY=`${API}/api/vision/identify`,RESET=`${API}/api/reset`,STARTUP_GREETING=`${API}/api/startup-greeting`,REALTIME_ENRICH=`${API}/api/realtime/enrich`;
const NANAKO_OUTFIT_MODELS=Object.freeze({
  neutral:"./static/models/nanako_avatar.vrm?v=2.6.3",
  sports:"./static/models/nanako_sports.vrm?v=2.6.3",
  casual:"./static/models/nanako_casual_tshirt_shorts.vrm?v=2.6.3",
  swimwear:"./static/models/nanako_swimwear.vrm?v=2.6.3",
  teacher:"./static/models/nanako_teacher.vrm?v=2.6.3",
  kimono:"./static/models/nanako_kimono.vrm?v=2.6.3",
  elegant:"./static/models/nanako_elegant_black_sleeveless.vrm?v=2.6.3"
});
const NANAKO_SCENE_ASSETS=Object.freeze({
  beach:"beach.webp",classroom:"classroom.webp",living_room:"living_room.webp",
  gym:"gym.webp",bar:"bar.webp",temple:"temple.webp",
  kyoto_street_scene:"kyoto_street_scene.webp"
});
const NANAKO_SCENE_OUTFITS=Object.freeze({
  beach:"swimwear",classroom:"teacher",living_room:"casual",gym:"sports",
  bar:"elegant",temple:"kimono",kyoto_street_scene:"kimono"
});
const NANAKO_OUTFIT_DEFAULT_SCENES=Object.freeze({
  sports:"gym",casual:"living_room",swimwear:"beach",teacher:"classroom",
  kimono:"kyoto_street_scene",elegant:"bar"
});
let currentNanakoOutfit="neutral",pendingNanakoOutfit="";
function normalizeNanakoOutfit(value){const key=String(value||"").trim().toLowerCase();return Object.hasOwn(NANAKO_OUTFIT_MODELS,key)?key:""}
function applyNanakoOutfitAction(action){
  if(!action||String(action.action||"")!=="change_outfit")return false;
  const outfit=normalizeNanakoOutfit(action.outfit);
  if(!outfit||outfit===currentNanakoOutfit||outfit===pendingNanakoOutfit)return false;
  const renderer=window.nanako3DRenderer;
  if(!renderer?.switchModel){console.warn("[Nanako Wardrobe] renderer switch API unavailable");return false}
  pendingNanakoOutfit=outfit;
  const requested=renderer.switchModel(NANAKO_OUTFIT_MODELS[outfit],{outfit});
  if(!requested){pendingNanakoOutfit="";return false}
  console.log(`[Nanako Wardrobe] loading ${outfit} • reason=${String(action.reason||"activity_intent")}`);
  return true;
}
function applyNanakoSceneCostume(background,action){
  let scene=String(background?.scene||action?.scene||"").trim().toLowerCase();
  let outfit=normalizeNanakoOutfit(action?.outfit);
  if(scene&&NANAKO_SCENE_OUTFITS[scene])outfit=NANAKO_SCENE_OUTFITS[scene];
  if(!scene&&outfit)scene=NANAKO_OUTFIT_DEFAULT_SCENES[outfit]||"";
  if(scene){
    const asset=String(background?.asset||NANAKO_SCENE_ASSETS[scene]||"").trim();
    if(asset)applyBackgroundDecision({scene,asset});
  }
  if(outfit)applyNanakoOutfitAction({
    action:"change_outfit",outfit,scene,
    reason:String(action?.reason||`scene_binding:${scene||"outfit_default"}`)
  });
}
window.addEventListener("nanako3d-model-changed",async event=>{
  const outfit=normalizeNanakoOutfit(event?.detail?.outfit||pendingNanakoOutfit);
  if(outfit){currentNanakoOutfit=outfit;localStorage.setItem("nanako_current_outfit",outfit)}
  pendingNanakoOutfit="";
  // Every complete VRM has its own bind pose. Rebuild the FBX retargeted clips
  // against the newly active skeleton instead of reusing the previous model's
  // bind-pose corrections.
  bodyAnimationsLoaded=false;bodyAnimationsLoading=null;currentBodyMotion="";
  await loadProductionBodyAnimations();
  syncBodyMotionForFrame(window.__nanakoPending3DFrame||{body_motion:"neutral",emotion:"neutral"});
});
window.addEventListener("nanako3d-model-error",()=>{pendingNanakoOutfit=""});
...
const verifiedBuildMarker=document.getElementById("buildMarker");if(verifiedBuildMarker)verifiedBuildMarker.textContent=`NanaChat v${RELEASE_VERSION} • Qwen3-ASR-Flash + Qwen3.6 Plus + Fish Audio Streaming`;
...
const UI_THEME_KEY="nanako_ui_theme_v1";
const MEMORY_KEY="nanakoPersistentMemoryV1";
const USER_NAME_KEY="nanakoPersistentUserNameV1";
const SESSION_HISTORY_MAX=600,MEMORY_SEND_MAX=8,MEMORY_FACT_MAX=80,LEARNER_PREFERENCE_MAX=100,LEARNER_TOPIC_MAX=80,LEARNER_PROGRESS_MAX=100,LEARNER_INTERACTION_MAX=100;
const DEFAULT_PERSONALITY_STATE={version:3,turns:0,mood:{valence:.62,energy:.56,warmth:.72,playfulness:.58,irritation:.04,vulnerability:.16},relationship:{familiarity:.04,trust:.08,comfort:.08,respect:.60,affection:.10,attraction:.05,phase:"new",couple:false,confession_pending:false,romantic_ready_streak:0,positive_streak:0,negative_streak:0,supportive_signals:0,humor_signals:0,romantic_signals:0,darling_unlocked:false},last:{intent:"respond_directly",tone:"warm",signal:"neutral"},initiative:{recent_topics:[],future_events:[],recent_events:[],shared_experiences:[],questions_asked:0,last_question_key:""}};
function freshPersonalityState(){return JSON.parse(JSON.stringify(DEFAULT_PERSONALITY_STATE))}
const LEARNING_TIERS=["Initiate","Explorer","Adept","Specialist","Veteran"];
const DEFAULT_LEARNING_PROFILE={version:2,selected_level:"",tier_index:0,tier_name:"Initiate",xp:0,xp_to_next:1000,lifetime_xp:0,skills:{grammar:0,vocabulary:0,listening_comprehension:0,conversation:0,fluency:0,naturalness:0,storytelling:0,opinions_reasoning:0,functional_japanese:0,interaction:0},turns_scored:0,mission_history:[],active_missions:[],last_mission_date:"",mission_shuffle_seed:{beginner:0,intermediate:0,advanced:0},mission_cursor:{beginner:0,intermediate:0,advanced:0},last_xp_gain:0,last_xp_reasons:[],level_up_event:null,mastery_capped:false};
function freshLearningProfile(){return JSON.parse(JSON.stringify(DEFAULT_LEARNING_PROFILE))}
function sanitizeLearningProfile(raw){
  const base=freshLearningProfile();if(!raw||typeof raw!=="object")return base;
  const lv=String(raw.selected_level||"").toLowerCase();if(["beginner","intermediate","advanced"].includes(lv))base.selected_level=lv;
  base.tier_index=Math.max(0,Math.min(4,Math.trunc(Number(raw.tier_index)||0)));base.tier_name=LEARNING_TIERS[base.tier_index];
  base.xp=Math.max(0,Math.min(1000,Math.trunc(Number(raw.xp)||0)));base.lifetime_xp=Math.max(0,Math.trunc(Number(raw.lifetime_xp)||0));base.turns_scored=Math.max(0,Math.trunc(Number(raw.turns_scored)||0));
  if(raw.skills&&typeof raw.skills==="object")for(const k of Object.keys(base.skills))base.skills[k]=Math.max(0,Math.min(100,Number(raw.skills[k])||0));
  base.mission_history=Array.isArray(raw.mission_history)?raw.mission_history.map(Number).filter(Number.isFinite).slice(-200):[];
  let missions=Array.isArray(raw.active_missions)?raw.active_missions:[];if(!missions.length&&raw.active_mission&&typeof raw.active_mission==="object")missions=[raw.active_mission];
  base.active_missions=missions.slice(0,5).map(m=>({id:Number(m?.id)||0,level:String(m?.level||m?.band||""),text:String(m?.text||"").slice(0,300),progress:Math.max(0,Math.min(100,Number(m?.progress)||0)),completed:!!m?.completed,date:String(m?.date||"").slice(0,10)})).filter(m=>m.id&&m.text);
  base.last_mission_date=String(raw.last_mission_date||"").slice(0,10);
  for(const lvName of ["beginner","intermediate","advanced"]){base.mission_shuffle_seed[lvName]=Math.max(0,Math.trunc(Number(raw?.mission_shuffle_seed?.[lvName])||0));base.mission_cursor[lvName]=Math.max(0,Math.trunc(Number(raw?.mission_cursor?.[lvName])||0));}
  base.mastery_capped=!!raw.mastery_capped;base.last_xp_gain=Math.max(0,Math.trunc(Number(raw.last_xp_gain)||0));base.last_xp_reasons=Array.isArray(raw.last_xp_reasons)?raw.last_xp_reasons.map(String).slice(0,6):[];base.level_up_event=raw.level_up_event&&typeof raw.level_up_event==="object"?raw.level_up_event:null;return base;
}
function sanitizePersonalityState(raw){
  const base=freshPersonalityState();if(!raw||typeof raw!=="object")return base;
  const num=(v,f)=>Number.isFinite(Number(v))?Math.max(0,Math.min(1,Number(v))):f;
  base.turns=Math.max(0,Math.min(100000,Math.trunc(Number(raw.turns)||0)));
  for(const k of Object.keys(base.mood))base.mood[k]=num(raw?.mood?.[k],base.mood[k]);
  for(const k of ["familiarity","trust","comfort","respect","affection","attraction"])base.relationship[k]=num(raw?.relationship?.[k],base.relationship[k]);
  for(const k of ["couple","confession_pending","darling_unlocked"])base.relationship[k]=!!raw?.relationship?.[k];
  for(const k of ["romantic_ready_streak","positive_streak","negative_streak","supportive_signals","humor_signals","romantic_signals"])base.relationship[k]=Math.max(0,Math.min(9999,Math.trunc(Number(raw?.relationship?.[k])||0)));
  const phase=String(raw?.relationship?.phase||base.relationship.phase);if(["new","acquaintance","familiar","friend","close","romantic_interest","couple"].includes(phase))base.relationship.phase=phase;
  for(const k of ["intent","tone","signal"]){const v=String(raw?.last?.[k]||"").trim().slice(0,48);if(v)base.last[k]=v}
  const init=raw?.initiative&&typeof raw.initiative==="object"?raw.initiative:{};for(const [k,n] of [["recent_topics",18],["future_events",12],["recent_events",12],["shared_experiences",12]])base.initiative[k]=(Array.isArray(init[k])?init[k]:[]).map(v=>String(v||"").trim().slice(0,180)).filter(Boolean).slice(-n);base.initiative.questions_asked=Math.max(0,Math.trunc(Number(init.questions_asked)||0));base.initiative.last_question_key=String(init.last_question_key||"").slice(0,80);
  return base;
}
let persistentFacts=[],learnerMemory={preferences:[],topic_affinities:[],language_progress:[],interaction_patterns:[]},nanakoPersonalityState=freshPersonalityState(),learningProfile=freshLearningProfile(),persistentUserName="",startupGreetingData=null,startupGreetingPlayed=false,startupGreetingLoading=null,startupGreetingPlayPromise=null;
function loadPersistentMemory(){
  try{
    ...
    learningProfile=sanitizeLearningProfile(raw?.learning_profile);
    learnerMemory={
  ...
  }catch(err){console.warn("[Nanako Memory] restore failed",err);history.length=0;persistentFacts=[];learnerMemory={preferences:[],topic_affinities:[],language_progress:[],interaction_patterns:[]};nanakoPersonalityState=freshPersonalityState();learningProfile=freshLearningProfile();persistentUserName="";}
}
...
function currentPersistentProfile(resetComplete=false){
  return {version:10,updated_at:Date.now(),reset_complete:!!resetComplete,facts:persistentFacts.slice(-MEMORY_FACT_MAX),learner_memory:learnerMemory,personality_state:nanakoPersonalityState,learning_profile:learningProfile,profile:{user_name:persistentUserName||""}};
}
...
function profileHasUsefulMemory(raw){
  ...
  if(["beginner","intermediate","advanced"].includes(String(raw?.learning_profile?.selected_level||"").toLowerCase()))return true;return Number(raw?.personality_state?.turns||0)>0;
}
...
async function saveProfileToFirestore({force=false}={}){
  ...
  const signature=JSON.stringify({facts:profile.facts,learner_memory:profile.learner_memory,personality_state:profile.personality_state,learning_profile:profile.learning_profile,profile:profile.profile});
...
async function syncPersistentMemoryFromServer(){
  ...
  try{
    ...
    firestoreLastSaveSignature=JSON.stringify({facts:persistentFacts.slice(-MEMORY_FACT_MAX),learner_memory:learnerMemory,personality_state:nanakoPersonalityState,learning_profile:learningProfile,profile:{user_name:persistentUserName||""}});
...
function mergeMemoryFacts(items){
  ...
  for(const item of items){
    ...
    if(/^The learner(?:'s|’s) favorite (?:video )?game is\s+/i.test(fact)){
      persistentFacts=persistentFacts.filter(x=>!/^The learner(?:'s|’s) favorite (?:video )?game is\s+/i.test(String(x||"").trim()));
    }
  }
...
function memoryPayload(){
  ...
  return {
    ...
    personality_state:nanakoPersonalityState,
...
async function acquireScreenWakeLock(){
  if(!active||document.visibilityState!=="visible"||!("wakeLock" in navigator)||screenWakeLock)return false;
  try{
    const lock=await navigator.wakeLock.request("screen");
    if(!active){await lock.release();return false}
    screenWakeLock=lock;
    lock.addEventListener("release",()=>{if(screenWakeLock===lock)screenWakeLock=null},{once:true});
    console.log("[Nanako] Screen wake lock active for conversation");
    return true;
  }catch(err){console.info("[Nanako] Screen wake lock unavailable; conversation continues normally",err);return false}
}
...
document.addEventListener("visibilitychange",()=>{
  if(document.visibilityState==="visible"&&active&&!screenWakeLock)void acquireScreenWakeLock();
});
function applyBackgroundDecision(value){
  const scene=String(value?.scene||"").trim(),asset=String(value?.asset||"").trim();
  if(!scene||!asset||scene===currentBackground||!e.background)return false;
  const next=`./static/backgrounds/${asset}?v=${CLIENT_BUILD}`;
  const img=e.background;
  const reveal=()=>{currentBackground=scene;img.classList.add("scene-ready");console.log(`[Background Director] loaded ${scene} -> ${asset}`)};
  img.classList.remove("scene-ready");
  const probe=new Image();
  probe.onload=()=>{img.src=next;requestAnimationFrame(reveal)};
  probe.onerror=()=>console.warn(`[Background Director] failed to load ${next}`);
  probe.src=next;
  return true;
}
...
const status=t=>e.status.textContent=t,clamp=(v,a,b)=>Math.max(a,Math.min(b,v)),label=l=>!l?"Choose level":String(l).charAt(0).toUpperCase()+String(l).slice(1);
function updateLevelDisplay(){const tier=learningProfile?.tier_name||LEARNING_TIERS[Number(learningProfile?.tier_index)||0]||"Initiate",shown=level?`${label(level)} · ${tier}`:"Choose level";e.levelBadge.textContent=shown;e.levelValue.textContent=level?label(level):"Choose level";if(e.settingsTier)e.settingsTier.textContent=level?`${label(level)} · ${tier} • ${Math.round(Number(learningProfile?.lifetime_xp)||0)} lifetime XP`:"Choose a level to begin."}
...
function setScore(v){score=clamp(Math.round(Number(v)||0),0,1000);let w=`${score/10}%`;e.scoreText.textContent=`${score} / 1000`;e.scoreFill.style.width=w;e.settingsScore.textContent=`${score} / 1000`;e.settingsScoreFill.style.width=w}
function renderLearningProgress(){level=String(learningProfile?.selected_level||level||"").toLowerCase();if(!["beginner","intermediate","advanced"].includes(level))level="";setScore(Number(learningProfile?.xp)||0);updateLevelDisplay();if(e.levelGrid)e.levelGrid.querySelectorAll("[data-level]").forEach(b=>b.classList.toggle("active",b.dataset.level===level));}
function mergeLearningProfile(value){if(!value||typeof value!=="object")return null;learningProfile=sanitizeLearningProfile(value);level=learningProfile.selected_level||level;renderLearningProgress();savePersistentMemory();return learningProfile.level_up_event||null}
function clientLocalDate(){const d=new Date(),p=n=>String(n).padStart(2,"0");return `${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())}`}
async function prepareDailyLearningProfile(){if(!level)return false;try{const r=await fetch(LEARNING_PREPARE,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_build:CLIENT_BUILD,level,client_local_date:clientLocalDate(),learning_profile:learningProfile})});const d=await jsonResp(r);if(!d?.ok||!d?.learning_profile)throw new Error(d?.error||"Daily challenge preparation failed");mergeLearningProfile(d.learning_profile);return true}catch(err){console.warn("[Nanako Learning] daily challenge preparation failed",err);return false}}
...
function syncCorrectionHelpers(){const x=currentCorrection;if(!x||e.corr.hidden)return;if(e.correctRO){e.correctRO.textContent=x.romaji;e.correctRO.hidden=!(showRO&&x.romaji)}if(e.correctEN){e.correctEN.textContent=x.english;e.correctEN.hidden=!(showEN&&x.english)}}
function showCorrection(x){clearTimeout(correctionTimer);if(!x.n||!x.o||!x.c){currentCorrection=null;e.corr.hidden=true;return}currentCorrection=x;e.wrong.textContent=x.o;e.correct.textContent=`→ ${x.c}`;e.corr.hidden=false;syncCorrectionHelpers();correctionTimer=setTimeout(()=>{e.corr.hidden=true;currentCorrection=null},7000)}
...
async function enrichStandardTurn(userText,replyText,userEntry,assistantEntry){
  ...
  // The tutor check is independent of the display toggles. A Japanese turn
  // must still be assessed and retained in history when RO and EN are hidden.
  // The same single enrichment also fills both translations for later toggles.
  const needCorrection=!!userEntry&&containsJapaneseForUi(userEntry.text)&&!userEntry.corrected;
  ...
  if(!needCorrection&&!needUser&&!needAssistant)return;
  const correctionOnly=needCorrection&&!needUser&&!needAssistant;
  if((userEntry&&userEntry.enrichmentPending)||(assistantEntry&&assistantEntry.enrichmentPending))return;
  if(userEntry)userEntry.enrichmentPending=true;
  if(assistantEntry)assistantEntry.enrichmentPending=true;
  ...
  try{
    const response=await fetch(REALTIME_ENRICH,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_build:CLIENT_BUILD,user_text:String(userText||""),reply_text:correctionOnly?"":String(replyText||""),current_background:currentBackground,correction_only:correctionOnly})});
    const data=await jsonResp(response);
    applyNanakoSceneCostume(data?.background,data?.outfit_action);
    if(userEntry&&history.includes(userEntry)){
      if(!userEntry.romaji)userEntry.romaji=String(data.user_romaji||"");
      if(!userEntry.english)userEntry.english=String(data.user_english||"");
      const x=correction(data);if(x.n){userEntry.wrong=x.o;userEntry.corrected=x.c;userEntry.correctionRomaji=x.romaji;userEntry.correctionEnglish=x.english;showCorrection(x)}
    }
    if(assistantEntry&&history.includes(assistantEntry)){
    ...
    if(userEntry)userEntry.enrichmentPending=false;if(assistantEntry){assistantEntry.translationPending=false;assistantEntry.enrichmentPending=false}renderHistory();savePersistentMemory();
    console.log("[Nanako Translation] missing romaji/English fields enriched asynchronously");
  }catch(err){if(userEntry)userEntry.enrichmentPending=false;if(assistantEntry){assistantEntry.translationPending=false;assistantEntry.enrichmentPending=false;if(history[history.length-1]===assistantEntry)showTranslationStatus("Romaji unavailable — tap RO to retry.","Translation unavailable — tap EN to retry.");renderHistory()}console.warn("[Nanako Translation] async enrichment failed",err)}
}
...
async function retryMissingHistoryTranslations(){
  if(historyTranslationSweepBusy)return;historyTranslationSweepBusy=true;
  try{
    const processed=new Set();
    for(let i=0;i<history.length;i++){
      const entry=history[i];if(!entry||processed.has(entry))continue;
      if(entry.role==="user"){
        let assistantEntry=null;for(let j=i+1;j<history.length;j++){if(history[j]?.role==="user")break;if(history[j]?.role==="assistant"&&!history[j]?.ephemeral){assistantEntry=history[j];break}}
        const needUser=containsJapaneseForUi(entry.text)&&((showRO&&!entry.romaji)||(showEN&&!entry.english));
        const needAssistant=!!assistantEntry&&containsJapaneseForUi(assistantEntry.text)&&((showRO&&!assistantEntry.romaji)||(showEN&&!assistantEntry.english));
        if(needUser||needAssistant)await enrichStandardTurn(entry.text,assistantEntry?.text||"",entry,assistantEntry);
        processed.add(entry);if(assistantEntry)processed.add(assistantEntry);continue;
      }
      if(entry.role==="assistant"&&containsJapaneseForUi(entry.text)&&((showRO&&!entry.romaji)||(showEN&&!entry.english))){await enrichStandardTurn("",entry.text,null,entry);processed.add(entry)}
    }
  }finally{historyTranslationSweepBusy=false}
}
function retryLatestTranslation(){void retryMissingHistoryTranslations()}
function currentUiTheme(){return document.documentElement.dataset.theme==="dark"?"dark":"light"}
function applyUiTheme(theme,{persist=true}={}){
  const next=theme==="dark"?"dark":"light";
  document.documentElement.dataset.theme=next;
  if(persist){try{localStorage.setItem(UI_THEME_KEY,next)}catch{}}
  const dark=next==="dark";
  if(e.themeIcon)e.themeIcon.textContent=dark?"☀︎":"☾";
  if(e.theme){e.theme.setAttribute("aria-label",dark?"Switch to light mode":"Switch to dark mode");e.theme.title=dark?"Switch to light mode":"Switch to dark mode"}
  const meta=document.querySelector('meta[name="theme-color"]');if(meta)meta.content=dark?"#090810":"#f2f5fa";
}
function toggleUiTheme(){applyUiTheme(currentUiTheme()==="dark"?"light":"dark")}
...
function localReturningStartupGreeting(name){
  const safe=cleanNameCandidate(name),prefix=safe?`${safe}、`:"";
  const greetings=[
    `${prefix}また会えてうれしいよ。今日は元気？`,
    `${prefix}来てくれてありがとう。今、どんな気分？`,
    `${prefix}今日も話せてうれしいな。どんな一日だった？`,
    `${prefix}また来てくれたんだね。ゆっくり話そう？`,
    `${prefix}ここに来てくれてありがとう。今、元気にしてる？`
  ];
...
async function fetchStartupGreeting(){
  ...
  startupGreetingLoading=(async()=>{
    try{
      ...
      const r=await fetch(STARTUP_GREETING,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...memoryPayload(),user_name:userName||"",avoid_greeting_id:avoidGreetingId})});
      const d=await jsonResp(r);
      ...
      const fallback=rememberDetectedUserName()?localReturningStartupGreeting(rememberDetectedUserName()):"NanaChatへようこそ。ななこです。よろしくね！";
...
async function playStartupGreetingIfReady(fromGesture=false){
  ...
  startupGreetingPlayPromise=(async()=>{
    let d=startupGreetingData;
    if(!d){
      const n=rememberDetectedUserName();
      d={reply:n?localReturningStartupGreeting(n):"NanaChatへようこそ。ななこです。よろしくね！",tts_stream:true,tts_voice_style:"normal",tts_sample_rate:24000,animation_plan:null};
      console.log("[Nanako Startup] immediate local greeting -> Fish stream");
      // Refresh randomized metadata in the background; never block first speech on it.
      void fetchStartupGreeting();
    }
    let ok=false;
...
function unlockAudio(){
  ...
  try{
    // Step 2.19.5: Fish playback uses Web Audio, not the HTMLAudio element.
    // iOS requires the AudioContext itself to be created/resumed from the
    // physical Enter tap. Prime both paths synchronously so the later network
    // streamed startup greeting is allowed to make sound.
    const AC=window.AudioContext||window.webkitAudioContext;
    if(AC){
      fishAudioCtx=fishAudioCtx||new AC();
      try{fishAudioCtx.resume()}catch{}
      try{const sfxAc=animationSfxContext();if(sfxAc&&sfxAc.state!=="running")sfxAc.resume()}catch{}
      try{
        const b=fishAudioCtx.createBuffer(1,1,fishAudioCtx.sampleRate||24000);
        const src=fishAudioCtx.createBufferSource();src.buffer=b;src.connect(fishAudioCtx.destination);src.start(0);
      }catch{}
    }
...
async function playFishStreaming(text,animationPlan=null,options={}){
  ...
  const mediaClock={duration:0,ended:false,get currentTime(){return Number.isFinite(streamClockStart)?Math.max(0,fishAudioCtx.currentTime-streamClockStart):0},pause(){try{ctrl.abort()}catch{};for(const src of fishScheduledSources.splice(0)){try{src.stop()}catch{}};this.ended=true;},load(){},src:""};
  currentAudio=mediaClock;micQueue=[];micBatch=[];micCapturePaused=true;convButton();
  let nextStart=NaN,totalSamples=0,pending=new Uint8Array(0),failed=false,talkingAnimationStarted=false,fallbackPlayed=false;
  const startAudibleTurn=(when)=>{
    if(!Number.isFinite(streamClockStart))streamClockStart=when;
    if(talkingAnimationStarted)return;
    talkingAnimationStarted=true;
    liveStreamMouthOverride="closed";
    useTalkingAnimation(animationPlan,mediaClock);
    void setServerNanakoSpeaking(true);
    status("Nanako is speaking...");
  };

  // Step 2.19.5: VRoid viseme lip-sync.  Fish still streams raw PCM, but mouth
  ...
  const lipWindowSamples=Math.max(1,Math.round(sampleRate*0.070));
  const lipVisualLead=0.050;
  const scheduleLipFrame=(mouth,when)=>setTimeout(()=>{
    if(currentAudio===mediaClock&&!mediaClock.ended)liveStreamMouthOverride=mouth;
  },Math.max(0,(when-lipVisualLead-fishAudioCtx.currentTime)*1000));
  // The first closed-mouth frame is scheduled only once the first audible PCM
  // has a real AudioContext start time (see startAudibleTurn).
  ...
  const schedulePcm=(bytes)=>{
    ...
    const when=Number.isFinite(nextStart)?Math.max(nextStart,fishAudioCtx.currentTime+0.010):(fishAudioCtx.currentTime+0.018);
    if(!talkingAnimationStarted){startAudibleTurn(when);scheduleLipFrame("closed",when);}
    src.start(when);nextStart=when+buf.duration;totalSamples+=n;mediaClock.duration=totalSamples/sampleRate;
    feedLipSamples(floats,when);
  };
  const scheduleFishWavFallback=async()=>{
    const fallbackResponse=await fetch(`${API}/api/speak`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text,voice_style:voiceStyle,client_build:CLIENT_BUILD}),signal:ctrl.signal});
    const fallbackData=await jsonResp(fallbackResponse);
    const b64=String(fallbackData?.audio_base64||"");
    if(!b64)throw new Error(fallbackData?.error||"Fish Audio returned no speech audio.");
    const raw=atob(b64),bytes=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)bytes[i]=raw.charCodeAt(i);
    const decoded=await fishAudioCtx.decodeAudioData(bytes.buffer.slice(0));
    const source=fishAudioCtx.createBufferSource();source.buffer=decoded;source.connect(fishAudioCtx.destination);fishScheduledSources.push(source);
    const when=Number.isFinite(nextStart)?Math.max(nextStart,fishAudioCtx.currentTime+0.018):(fishAudioCtx.currentTime+0.018);
    if(!talkingAnimationStarted){startAudibleTurn(when);scheduleLipFrame("closed",when);}
    source.start(when);nextStart=when+decoded.duration;
    totalSamples=Math.max(1,Math.round(decoded.duration*sampleRate));mediaClock.duration=decoded.duration;
    feedLipSamples(decoded.getChannelData(0),when);fallbackPlayed=true;
    console.log(`[Fish Stream] WAV recovery scheduled ${decoded.duration.toFixed(2)}s`);
  };
  try{
    ...
    if(!r.ok){
      ...
      console.warn(`[Fish Stream] HTTP ${r.status}; trying the complete-audio recovery path`,detail.slice(0,240));
      await scheduleFishWavFallback();
    }else{
    ...
    if(totalSamples===0&&!fallbackPlayed&&!ctrl.signal.aborted){
      ...
      // in the SAME WebAudio/animation turn.  Normal turns remain true streaming.
      console.warn("[Fish Stream] zero PCM bytes; retrying once with verified Fish WAV fallback");
      status("Nanako is speaking...");
      await scheduleFishWavFallback();
    }
    ...
    // which pauses with the audio device and resumes when the app returns.
    while(!ctrl.signal.aborted&&Number.isFinite(nextStart)&&fishAudioCtx.currentTime<nextStart+0.025){
      if(document.visibilityState==="visible"&&fishAudioCtx.state!=="running"){
        try{await fishAudioCtx.resume()}catch{}
      }
    ...
    await setServerNanakoSpeaking(false);micCapturePaused=!!options?.suppressResume;stopAnimationPlan();
    if(document.visibilityState!=="visible"){
      // Background lifecycle owns recovery. Never restart mic/animation while iOS
      // has the PWA suspended.
      returnToPythonIdle({emotion:idleEmotion});
      convButton();
    }else{
      finishPlanWithPostHold(animationPlan,{idleEmotion,onDone:()=>{if(options?.suppressResume){status("Nanako is celebrating...");return;}if(active){status("I am listening...");setTimeout(begin,20)}else status("Ready to chat")}});convButton();
    }
  }
  return !failed;
}
...
async function play(b,m,animationPlan=null,options={}){
  ...
  const finish=()=>{
    ...
    finishPlanWithPostHold(animationPlan,{idleEmotion,onDone:()=>{
      if(active){micCapturePaused=false;status("I am listening...");setTimeout(begin,20)}
...
function ingestLocalVad(samples){
  ...
  if(localVadSilenceMs>=1800||localVadSpeechMs>=20000)finishLocalVadTurn();
}
...
async function recoverDigitallySilentMic(){
  ...
  try{
    ...
    if(active&&!micCapturePaused)status("I am listening...");
  }catch(err){
...
async function pumpMicQueue(){
  ...
  try{
    while(active&&micSessionId&&micQueue.length){
      ...
      if(m.speech_started){userSpeechActive=true;window.nanakoBackgroundMusic?.duck("user-turn",true);status("I'm listening...");if(cameraVoiceMode)e.cameraStatus.textContent="I can hear you..."}
      if(m.turn_id){
        window.nanakoBackgroundMusic?.duck("user-turn",true);
  ...
  try{
    ...
    if(cameraVoiceMode&&!e.cameraModal.hidden){payload.image_data_url=captureCameraFrame();payload.trigger="rear_object_camera";e.cameraStatus.textContent="Nanako is looking and listening..."}
      ...
      if(awarenessActive){
        ...
        payload.trigger="front_camera_request";payload.visual_target=String(inspection.visual_target||"face_or_scene");frontVisionAttached=true;console.log(`[Nanako Vision 12.0.139] one authorized CURRENT front frame attached • target=${payload.visual_target} • chars=${payload.image_data_url.length}`);status("Nanako is looking...")
      }
    ...
    if(d?.ignored&&!String(d?.transcript||"").trim()){
      emptyTranscriptTurn=true;micCapturePaused=false;
      console.info("[Nanako Mic] ASR returned no words; keeping the conversation active for another attempt.");
      return;
    }
    ...
    if(activeMicTurnController===turnController){
      activeMicTurnController=null;busy=false;userSpeechActive=false;
      window.nanakoBackgroundMusic?.duck("user-turn",false);
      if(active&&!currentAudio){status(emptyTranscriptTurn?"I didn’t catch that—please try again.":"Listening...");setTimeout(begin,40)}
    }
  }
}
...
function applyRealtimePostState({emotion="neutral",bodyMotion="neutral",eyeGesture="none"}={}){
  ...
  if(wink){
    ...
    scheduleAnimationSfx({duration_ms:1560,sfx:{src:"./static/sfx/wink.wav?v=2.6.3",start_ms:560,stop_ms:1560,name:"wink-realtime",volume:1}});
    realtimeGestureTimer=setTimeout(()=>{
      ...
      realtimeGestureTimer=setTimeout(()=>{
        ...
        setTimeout(()=>returnToPythonIdle({emotion:realtimeEmotion,bodyMotion:realtimeBodyMotion}),400);
      },1000);
    },160);return;
  }
  if(realtimeBodyMotion==="clapping"){
    stopAnimationPlan();scheduleAnimationSfx({duration_ms:2600,sfx:{src:"./static/sfx/single_clap.wav?v=2.6.3",hits_ms:[140,432,724,1016,1308,1600,1892,2184,2476],hit_volumes:[1.00,0.60,0.82,0.48,0.74,0.56,0.90,0.64,0.78],stop_ms:2600,name:"single-clap-realtime",volume:1}});renderAnimationFrame({emotion:realtimeEmotion||"happy",action:"clapping",eyes:"open",mouth:"closed",body_motion:"clapping",scale:1,translate_y:0});
...
async function begin(){
  if(!active)return;
  /* v2.6.3: endpoint locally, then upload one complete turn. Function Compute
...
let pendingOnboardingLevel="";
function showLearningOnboarding(){
  if(e.learningOnboarding)e.learningOnboarding.hidden=false;
  if(e.dailyChallengesScreen)e.dailyChallengesScreen.hidden=true;
  pendingOnboardingLevel="";
  if(e.learningContinue)e.learningContinue.disabled=true;
  if(e.learningOnboarding)e.learningOnboarding.querySelectorAll("[data-onboarding-level]").forEach(b=>b.classList.remove("selected"));
}
function hideLearningOnboarding(){if(e.learningOnboarding)e.learningOnboarding.hidden=true;}
function hideDailyChallenges(){if(e.dailyChallengesScreen)e.dailyChallengesScreen.hidden=true;}
function renderDailyChallenges(){
  if(!e.dailyChallengesList)return;
  const missions=Array.isArray(learningProfile?.active_missions)?learningProfile.active_missions.slice(0,5):[];
  e.dailyChallengesList.innerHTML="";
  missions.forEach((mission,index)=>{
    const row=document.createElement("div");row.className="daily-challenge-item";
    const num=document.createElement("div");num.className="daily-challenge-number";num.textContent=String(index+1);
    const text=document.createElement("div");text.className="daily-challenge-text";text.textContent=String(mission?.text||"");
    row.append(num,text);e.dailyChallengesList.appendChild(row);
  });
  while(e.dailyChallengesList.children.length<5){
    const row=document.createElement("div");row.className="daily-challenge-item";
    row.innerHTML=`<div class="daily-challenge-number">${e.dailyChallengesList.children.length+1}</div><div class="daily-challenge-text">Preparing challenge…</div>`;
    e.dailyChallengesList.appendChild(row);
  }
}
function renderDailyChallengesReminder(){
  if(!e.dailyChallengesReminderList)return;
  const missions=Array.isArray(learningProfile?.active_missions)?learningProfile.active_missions.slice(0,5):[];
  e.dailyChallengesReminderList.innerHTML="";
  missions.forEach((mission,index)=>{
    const row=document.createElement("div");row.className=`daily-reminder-item${mission?.completed?" completed":""}`;
    const n=document.createElement("div");n.className="daily-reminder-number";n.textContent=mission?.completed?"✓":String(index+1);
    const wrap=document.createElement("div"),copy=document.createElement("div"),progress=document.createElement("div");copy.className="daily-reminder-copy";copy.textContent=String(mission?.text||"");progress.className="daily-reminder-progress";progress.textContent=mission?.completed?"Completed ✓":`Progress ${Math.max(0,Math.min(100,Number(mission?.progress)||0))}%`;wrap.append(copy,progress);row.append(n,wrap);e.dailyChallengesReminderList.appendChild(row);
  });
  if(!missions.length)e.dailyChallengesReminderList.innerHTML='<div class="history-empty">No challenges prepared yet.</div>';
}
function randomIdleQuestionDelayMs(){return 60000;}
function revealInteractionSurface(){interactionStarted=true;document.body.classList.remove("preinteraction");lastHumanActivityAt=Date.now();idleQuestionDueAt=lastHumanActivityAt+randomIdleQuestionDelayMs();startIdleQuestionWatchdog();scheduleIdleQuestion();}
function noteHumanActivity(){lastHumanActivityAt=Date.now();if(idleQuestionPending)idleQuestionPending=false;idleQuestionDueAt=lastHumanActivityAt+randomIdleQuestionDelayMs();scheduleIdleQuestion();}
function clearIdleQuestionTimer(){if(idleQuestionTimer){clearTimeout(idleQuestionTimer);idleQuestionTimer=0;}}
function startIdleQuestionWatchdog(){if(idleQuestionWatchdog)return;idleQuestionWatchdog=setInterval(()=>{if(interactionStarted&&!document.hidden&&!idleQuestionPending&&idleQuestionDueAt&&Date.now()>=idleQuestionDueAt)void maybeAskIdleQuestion();},15000);}
function stopIdleQuestionWatchdog(){if(idleQuestionWatchdog){clearInterval(idleQuestionWatchdog);idleQuestionWatchdog=0;}}
function scheduleIdleQuestion(){
  clearIdleQuestionTimer();if(!interactionStarted||document.hidden||idleQuestionPending)return;
  if(!idleQuestionDueAt)idleQuestionDueAt=Math.max(lastHumanActivityAt,Date.now())+randomIdleQuestionDelayMs();
  const remaining=Math.max(250, idleQuestionDueAt-Date.now());
  idleQuestionTimer=setTimeout(()=>void maybeAskIdleQuestion(),Math.min(remaining,30000));
}
function idleQuestionTemporarilyBlocked(){return !!(busy||userSpeechActive||currentAudio||!level||document.hidden||!e.settings?.hidden||!e.historyModal?.hidden||!e.dailyChallengesModal?.hidden||!e.cameraModal?.hidden);}
async function maybeAskIdleQuestion(){
  clearIdleQuestionTimer();
  if(!interactionStarted||idleQuestionPending||idleQuestionRequestBusy||document.hidden)return;
  if(Date.now()<idleQuestionDueAt){scheduleIdleQuestion();return;}
  // If Nanako is momentarily busy at the deadline, retry soon instead of
  // drawing a brand-new 1-minute delay. This fixes idle questions being
  // starved by speech/audio/modal transitions.
  if(idleQuestionTemporarilyBlocked()){idleQuestionTimer=setTimeout(()=>void maybeAskIdleQuestion(),12000);return;}
  idleQuestionRequestBusy=true;
  try{
    console.log(`[Nanako Initiative] idle deadline reached after ${Math.round((Date.now()-lastHumanActivityAt)/1000)}s`);
    const r=await fetch(IDLE_QUESTION,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_build:CLIENT_BUILD,personality_state:nanakoPersonalityState,memory_facts:persistentFacts.slice(-MEMORY_FACT_MAX),learner_memory:learnerMemory})});
    const d=await jsonResp(r);const q=String(d?.question||"").trim();if(!d?.ok||!q)throw new Error("No idle question returned");
    mergePersonalityState(d.personality_state);idleQuestionPending=true;idleQuestionDueAt=0;e.jp.textContent=q;e.roText.textContent="";e.enText.textContent="";addHistory("assistant",q,null,{});renderHistory();savePersistentMemory();void saveProfileToFirestore();status("Nanako has a question...");
    let spoken=false;if(!muted)spoken=await playFishStreaming(q,null,{idleEmotion:"happy",voiceStyle:String(d?.tts_voice_style||"normal"),sampleRate:Number(d?.tts_sample_rate||24000)});
    if(!spoken){returnToPythonIdle({emotion:"happy"});if(active)setTimeout(begin,40);}
    // Intentionally do not arm another question. Only a real learner turn
    // calls noteHumanActivity(), clears pending, and starts a fresh 1-minute window.
  }catch(err){console.warn("[Nanako Initiative] idle question failed",err);idleQuestionDueAt=Date.now()+30000;scheduleIdleQuestion();}
  finally{idleQuestionRequestBusy=false;}
}

async function showDailyChallenges(){
  hideLearningOnboarding();
  if(!level)return showLearningOnboarding();
  if(e.dailyChallengesScreen){e.dailyChallengesScreen.hidden=false;e.dailyChallengesScreen.classList.add("loading");}
  renderDailyChallenges();
  await prepareDailyLearningProfile();
  renderDailyChallenges();renderDailyChallengesReminder();
  if(e.dailyChallengesScreen)e.dailyChallengesScreen.classList.remove("loading");
  status("Daily challenges ready");
}
async function finishStartupGreeting(){
  hideLearningOnboarding();hideDailyChallenges();status("Nanako is greeting you...");cachedPythonIdlePlan=null;
  try{nanakoAvatar=nanakoAvatar||document.getElementById("nanakoAvatar");nanakoMotion=nanakoMotion||document.querySelector(".nanako-motion");renderAnimationFrame({emotion:"neutral",action:"idle",eyes:"open",mouth:"closed",scale:1,translate_y:0});if(!startupGreetingData)await fetchStartupGreeting();if(startupGreetingData&&!muted){const ok=await playStartupGreetingIfReady(true);if(!ok)console.warn("[Nanako Startup] greeting data loaded but voice playback did not start");}}
  catch(err){console.error("[NanaChat Startup Enter]",err);error("Nanako's welcome voice could not play.");}
  finally{if(!currentAudio&&!active)status("Ready to chat");}
}
function selectOnboardingLevel(next){
  next=String(next||"").toLowerCase();if(!["beginner","intermediate","advanced"].includes(next))return;
  pendingOnboardingLevel=next;
  if(e.learningOnboarding)e.learningOnboarding.querySelectorAll("[data-onboarding-level]").forEach(b=>b.classList.toggle("selected",b.dataset.onboardingLevel===next));
  if(e.learningContinue)e.learningContinue.disabled=false;
}
async function continueOnboardingLevel(){
  const next=String(pendingOnboardingLevel||"").toLowerCase();if(!["beginner","intermediate","advanced"].includes(next))return;
  unlockAudio();level=next;learningProfile=freshLearningProfile();learningProfile.selected_level=next;renderLearningProgress();savePersistentMemory();
  await prepareDailyLearningProfile();await saveProfileToFirestore({force:true});await showDailyChallenges();
}
async function startFromDailyChallenges(){unlockAudio();hideDailyChallenges();revealInteractionSurface();await finishStartupGreeting();}
async function enterStartupSplash(){
  if(startupEnterDone)return;if(!window.nanako3DRenderer?.ready){const waitingButton=document.getElementById("startupEnterButton");if(waitingButton){waitingButton.disabled=true;waitingButton.textContent="Loading Nanako 3D…";}return;}startupEnterDone=true;const splash=document.getElementById("startupSplash"),enterBtn=document.getElementById("startupEnterButton");document.documentElement.classList.remove("splash-visible");void requestPortraitOrientationLock();unlockAudio();if(enterBtn){enterBtn.disabled=true;enterBtn.textContent="Entering…";}
  // Prepare the destination overlay before removing the splash. The app shell
  // remains visibility:hidden until Start with Nanako, eliminating the flash.
  if(!level){showLearningOnboarding();if(splash)splash.hidden=true;status("Choose your Japanese level");return;}
  const dailyPromise=showDailyChallenges();if(splash)splash.hidden=true;await dailyPromise;if(enterBtn){enterBtn.disabled=false;enterBtn.textContent="Enter";}
}
...
async function stopMode(){
  active=false;userSpeechActive=false;window.nanakoBackgroundMusic?.duck("user-turn",false);await setServerNanakoSpeaking(false);await stopRealtimeSession();await stopMicBridge();await stopAudio(false);
  await releaseScreenWakeLock();
  // Step 2.27.0: ending voice conversation must never leave the text composer
  ...
  convButton();status("Ready to chat");
}
async function reset(){if(!window.confirm("Forget everything — including Nanako’s relationship memory, Japanese XP, tier, daily challenges, level choice and progress — and start as a completely new user? This cannot be undone."))return;await stopVisualAwareness({silent:true});await stopMode();try{await fetch(RESET,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({client_build:CLIENT_BUILD,profile_id:SERVER_PROFILE_ID})})}catch{}history.length=0;persistentFacts=[];learnerMemory={preferences:[],topic_affinities:[],language_progress:[],interaction_patterns:[]};nanakoPersonalityState=freshPersonalityState();learningProfile=freshLearningProfile();level="";persistentUserName="";startupGreetingData=null;startupGreetingPlayed=false;startupGreetingLoading=null;startupGreetingPlayPromise=null;startupGestureArmed=false;startupEnterDone=false;try{localStorage.removeItem(USER_NAME_KEY);localStorage.setItem(MEMORY_KEY,JSON.stringify({version:10,updated_at:Date.now(),reset_complete:true,facts:[],learner_memory:learnerMemory,personality_state:nanakoPersonalityState,learning_profile:learningProfile,profile:{user_name:""}}))}catch{}firestoreLastSaveSignature="";firestoreMemoryReady=true;renderHistory();renderLearningProgress();e.jp.textContent="NanaChatへようこそ。ななこです。よろしくね！";e.roText.textContent=e.enText.textContent="";e.corr.hidden=e.settings.hidden=e.historyModal.hidden=true;if(e.dailyChallengesModal)e.dailyChallengesModal.hidden=true;interactionStarted=false;idleQuestionPending=false;idleQuestionDueAt=0;idleQuestionRequestBusy=false;clearIdleQuestionTimer();stopIdleQuestionWatchdog();document.body.classList.add("preinteraction");document.documentElement.classList.add("splash-visible");hideLearningOnboarding();hideDailyChallenges();const splash=document.getElementById("startupSplash");if(splash)splash.hidden=false;status("Ready for a fresh start");void fetchStartupGreeting()}
...
async function selectJapaneseLevel(nextLevel){
  const next=String(nextLevel||"").toLowerCase();if(!/^(?:beginner|intermediate|advanced)$/.test(next)||settingSwitchBusy)return;if(next===level){updateLevelDisplay();return}
  settingSwitchBusy=true;disableSettingButtons(true);try{level=next;autoEffectiveLevel="";learningProfile.selected_level=next;learningProfile.tier_index=0;learningProfile.tier_name="Initiate";learningProfile.xp=0;learningProfile.active_missions=[];learningProfile.last_mission_date="";renderLearningProgress();savePersistentMemory();await prepareDailyLearningProfile();await saveProfileToFirestore({force:true});await restartMicForSettingChange(`to ${label(level)}`);console.log(`[Nanako Level] ${label(level)} active • XP tier reset to Initiate`);}catch(err){console.error("[Nanako Level switch]",err);error("The level changed, but the microphone could not restart. Tap End Conversation, then Start Conversation.");status("Microphone restart needed");}finally{settingSwitchBusy=false;disableSettingButtons(false);}
}
...
function stopLocalSpeechForBackground(reason="background"){
  // Step 2.27.0 iOS/PWA lifecycle hard reset. iOS can freeze WebAudio, fetch,
  ...
  // locally and create a fresh mic session when the app returns.
  backgroundLifecycleEpoch+=1;
  const oldSession=micSessionId;
  try{activeMicTurnController?.abort()}catch{};activeMicTurnController=null;
  try{fishStreamAbort?.abort()}catch{};fishStreamAbort=null;
  for(const src of fishScheduledSources.splice(0)){try{src.stop()}catch{}}
  if(currentAudio){try{currentAudio.pause?.()}catch{};currentAudio=null;}
  liveStreamMouthOverride="";
  if(bargeCaptureTimer){clearTimeout(bargeCaptureTimer);bargeCaptureTimer=0;}
  micSessionGeneration+=1;micSessionId="";micQueue=[];micBatch=[];
  micCapturePaused=true;userSpeechActive=false;busy=false;
  stopAnimationPlan();returnToPythonIdle({emotion:"neutral"});convButton();
  // Best-effort server cleanup with keepalive. These calls are advisory; the
  // fresh foreground session does not depend on their completion.
  if(oldSession){
    try{fetch(MIC_SPEAKING,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({session_id:oldSession,speaking:false}),keepalive:true}).catch(()=>{})}catch{}
    try{fetch(MIC_STOP,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({session_id:oldSession}),keepalive:true}).catch(()=>{})}catch{}
  }
  console.log(`[Nanako Lifecycle] ${reason} hard-reset transient voice/animation state • epoch=${backgroundLifecycleEpoch}`);
}
...
async function recoverAfterForeground(){
  ...
  try{
    ...
    for(const ac of [fishAudioCtx,ctx,realtimeAudioContext,animationSfxCtx]){
      try{if(ac&&ac.state!=="running"&&ac.state!=="closed")await ac.resume()}catch(err){console.warn("[Nanako Foreground] AudioContext resume failed",err)}
    }
    if(currentAudio||fishStreamAbort||userSpeechActive){
      // A stale turn survived a WebKit lifecycle callback. Fail closed rather
      // than attempting to resume a half-suspended audio/animation timeline.
      stopLocalSpeechForBackground("foreground-stale-turn");
    }
    ...
    busy=false;userSpeechActive=false;micCapturePaused=false;
    stopAnimationPlan();returnToPythonIdle({emotion:"neutral"});
    status("I am listening...");
    console.log(`[Nanako Foreground] microphone + Python session recovered • epoch=${backgroundLifecycleEpoch}`);
  }catch(err){
...
document.addEventListener("visibilitychange",()=>{
  if(document.hidden){
    clearIdleQuestionTimer();
    stopLocalSpeechForBackground("visibility-hidden");
  }else{
    setTimeout(()=>void recoverAfterForeground(),100);
    if(interactionStarted){startIdleQuestionWatchdog();scheduleIdleQuestion();}
  }
});
window.addEventListener("pagehide",()=>stopLocalSpeechForBackground("pagehide"));
window.addEventListener("pageshow",()=>setTimeout(()=>void recoverAfterForeground(),120));
window.addEventListener("focus",()=>{if(document.visibilityState==="visible"){setTimeout(()=>void recoverAfterForeground(),140);if(interactionStarted){startIdleQuestionWatchdog();scheduleIdleQuestion();}}});
...
async function boot(){
  ...
  if(e.jp)e.jp.textContent=rememberDetectedUserName()?localReturningStartupGreeting(rememberDetectedUserName()):`NanaChatへようこそ。ななこです。よろしくね！`;
  renderAnimationFrame({emotion:"neutral",action:"idle",eyes:"open",mouth:"closed",scale:1,translate_y:0});
  ...
  console.log(`[NanaChat] v${RELEASE_VERSION} • QWEN3-ASR-FLASH + Qwen3.6 Plus + FISH-AUDIO-STREAMING runtime=${CLIENT_BUILD} • learner model: ${learnerMemory.preferences.length} preferences, ${learnerMemory.language_progress.length} language patterns, ${learnerMemory.interaction_patterns.length} interaction patterns • user=${persistentUserName||"unknown"}`);
}

boot();
})();

// v2.6.9 — One reason-based music duck controller. Voice activity and Nanako playback
// hold the duck through the user turn, response thinking, and complete spoken reply.
(()=>{
  const tracks=[
    "./static/music/midday-drift.mp3?v=mp3-02",
    "./static/music/midnight-in-kyoto.mp3?v=mp3-02",
    "./static/music/tokio-twilight.mp3?v=mp3-02",
    "./static/music/midnight-drift.mp3?v=mp3-02",
    "./static/music/soft-japanese-rain.mp3?v=mp3-02"
  ];
  let index=0,audio=null,audioContext=null,audioSource=null,audioGain=null,started=false,startPromise=null,startPromiseFromGesture=false,startAttemptId=0,fadeFrame=0;
  const ducks=new Set();
  const iosNeedsGesture=/iPad|iPhone|iPod/.test(navigator.userAgent)||(/MacIntel/.test(navigator.platform)&&navigator.maxTouchPoints>1);
  const MUSIC_ENABLED_KEY="nanako_background_music_enabled_v5";
  // Ignore saved-off values from earlier releases so this update starts with music enabled.
  let enabled=true;try{enabled=localStorage.getItem(MUSIC_ENABLED_KEY)!=="false"}catch{}
  // Idle music remains at 75%. During a user turn or Nanako speech, duck to
  // 40% of that level (30% absolute gain) and restore smoothly when all reasons clear.
  // Route through Web Audio because iPhone Safari ignores media-element volume.
  const normal=.75,ducked=.40,fadeMs=1400;
  const target=()=>!enabled?0:ducks.size?ducked:normal;
  const fadeTo=(value,ms=fadeMs)=>{
    if(!audio)return;
    if(fadeFrame)cancelAnimationFrame(fadeFrame);
    const targetAudio=audio,targetGain=audioGain,from=targetGain?Number(targetGain.gain.value)||0:Number(targetAudio.volume)||0,start=performance.now();
    const tick=now=>{
      if(audio!==targetAudio){fadeFrame=0;return;}
      const t=Math.min(1,(now-start)/ms);
      const next=from+(value-from)*t;
      if(targetGain)targetGain.gain.setValueAtTime(next,audioContext.currentTime);else targetAudio.volume=next;
      if(t<1)fadeFrame=requestAnimationFrame(tick);else fadeFrame=0;
    };
    fadeFrame=requestAnimationFrame(tick);
  };
  const playCurrent=async(fromGesture=false)=>{
    if(!enabled||!audio)return false;
    const playingAudio=audio;
    let resumePromise=null;
    if(audioContext&&audioContext.state!=="running"){
      try{resumePromise=audioContext.resume()}catch(err){console.warn("[Nanako BGM] AudioContext resume deferred",err);return false;}
      if(fromGesture){void resumePromise.catch(err=>console.warn("[Nanako BGM] AudioContext resume deferred",err));}
      else{
        try{await resumePromise}catch(err){console.warn("[Nanako BGM] Waiting for the first audio gesture",err);return false;}
        // Never let a muted/suspended media element advance the song before
        // iOS grants sound permission. The first gesture retries this same
        // element from time zero.
        if(audioContext.state!=="running")return false;
      }
    }
    if(!fromGesture&&audioContext&&audioContext.state!=="running")return false;
    try{const playPromise=playingAudio.play();await Promise.all([playPromise,resumePromise||Promise.resolve()]);if((audioContext&&audioContext.state!=="running")||!enabled||audio!==playingAudio){playingAudio.pause();return false;}fadeTo(target());return true;}catch(err){console.warn("[Nanako BGM] browser blocked playback",err);return false;}
  };
  const playNext=async(fromGesture=false)=>{
    if(!enabled)return false;
    if(fadeFrame)cancelAnimationFrame(fadeFrame);fadeFrame=0;
     if(audio){audio.onended=null;audio.pause();audio.src="";try{audioSource?.disconnect();audioGain?.disconnect()}catch{}audioSource=null;audioGain=null;}
    audio=new Audio(tracks[index]);audio.preload="auto";audio.volume=1;audio.loop=false;let fadingOut=false;
    try{
      const AC=window.AudioContext||window.webkitAudioContext;
       if(AC){audioContext=audioContext||new AC();audioSource=audioContext.createMediaElementSource(audio);audioGain=audioContext.createGain();audioGain.gain.value=0;audioSource.connect(audioGain);audioGain.connect(audioContext.destination);}
     }catch(err){try{audioSource?.disconnect();audioGain?.disconnect()}catch{}audioSource=null;audioGain=null;console.warn("[Nanako BGM] Web Audio gain unavailable; using media volume",err);audio.volume=0;}
    ...
    return await playCurrent(fromGesture);
  };
  const stopPlayback=()=>{started=false;startAttemptId++;startPromise=null;startPromiseFromGesture=false;if(fadeFrame)cancelAnimationFrame(fadeFrame);fadeFrame=0;if(audio){audio.pause();if(audioGain&&audioContext)audioGain.gain.setValueAtTime(0,audioContext.currentTime);else audio.volume=0;}};
  window.nanakoBackgroundMusic={
    start:async(fromGesture=false)=>{if(!enabled)return false;if(started&&audio&&!audio.paused){if(fromGesture&&audioContext&&audioContext.state!=="running")void audioContext.resume().catch(err=>console.warn("[Nanako BGM] AudioContext resume deferred",err));fadeTo(target(),240);return true;}if(startPromise){if(!fromGesture||startPromiseFromGesture)return startPromise;startAttemptId++;startPromise=null;}const attempt=++startAttemptId,pending=(async()=>{const ok=audio?await playCurrent(fromGesture):await playNext(fromGesture);if(attempt===startAttemptId)started=ok;return ok})();startPromise=pending;startPromiseFromGesture=!!fromGesture;try{return await pending}finally{if(startPromise===pending){startPromise=null;startPromiseFromGesture=false}}},
    duck:(reason,on)=>{if(on)ducks.add(reason);else ducks.delete(reason);fadeTo(target(),360);},
    isEnabled:()=>enabled,
    isAudioReady:()=>!audioContext||audioContext.state==="running",
    toggle:()=>{enabled=!enabled;try{localStorage.setItem(MUSIC_ENABLED_KEY,String(enabled))}catch{}if(enabled){if(!started)void window.nanakoBackgroundMusic.start(true);else fadeTo(target(),420)}else stopPlayback();document.dispatchEvent(new Event("nanako-music-state"));return enabled},
    stop:stopPlayback
  };
  ...
  // iPhone startup is handled by the splash click handler in index.html; do not
  // compete with it using pointerdown/touchstart playback attempts here.
  document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible"&&enabled&&audioContext&&audioContext.state!=="running")void audioContext.resume().catch(err=>console.warn("[Nanako BGM] AudioContext resume deferred",err))});
  // Avoid creating a suspended playback attempt on iPhone. The first splash
  // touch starts AudioContext and HTMLAudio together inside the gesture.
  if(!iosNeedsGesture)void window.nanakoBackgroundMusic.start();
})();
