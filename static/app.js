...
// Step 2.09 release gate is created inline by index.html, before any external
...
async function ensureCurrentServiceWorker(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("./sw.js?v=12.0.9",{scope:"./",updateViaCache:"none"});
    await reg.update();
...
// ============================================================
// NANAKO v11 STEP 2.09 — FAST TURN CONTRACT + COMPLETE HISTORY TRANSLATIONS
...
async function loadProductionBodyAnimations(){
  ...
  bodyAnimationsLoading=(async()=>{
    const result=await window.nanako3DRenderer.loadBodyAnimations({
      neutral:"./static/animations/nanako_idle.fbx?v=12.0.9",
      angry:"./static/animations/nanako_angry.fbx?v=12.0.9",
      thinking:"./static/animations/nanako_thinking.fbx?v=12.0.9",
      clapping:"./static/animations/nanako_clapping.fbx?v=12.0.9"
    });
    ...
    console.log("[Nanako 3D] Step 2.09 body animations ready: neutral, angry, thinking, clapping");
    return true;
...
const CLIENT_BUILD="12.0.9",RELEASE_VERSION="2.09",API="https://nanako-web-pokbkohedy.ap-southeast-1.fcapp.run",CHAT=`${API}/api/chat`,VISION_IDENTIFY=`${API}/api/vision/identify`,RESET=`${API}/api/reset`,STARTUP_GREETING=`${API}/api/startup-greeting`;
  ...
  try{
    ...
    if(cameraVoiceMode&&!e.cameraModal.hidden){payload.image_data_url=captureCameraFrame();payload.trigger="rear_object_camera";e.cameraStatus.textContent="Nanako is looking and listening..."}
      ...
      if(awarenessActive){
        ...
        payload.trigger="front_camera_request";payload.visual_target=String(inspection.visual_target||"face_or_scene");frontVisionAttached=true;console.log(`[Nanako Vision 12.0.9] one authorized front frame attached • target=${payload.visual_target} • chars=${payload.image_data_url.length}`);status("Nanako is looking...")
      }
