...
// Step 1.99 release gate is created inline by index.html, before any external
...
async function ensureCurrentServiceWorker(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("./sw.js?v=11.9.9",{scope:"./",updateViaCache:"none"});
    await reg.update();
...
// ============================================================
// NANAKO v11 STEP 1.99 — FAST SPEECH-END RESPONSE
...
async function loadProductionBodyAnimations(){
  ...
  bodyAnimationsLoading=(async()=>{
    const result=await window.nanako3DRenderer.loadBodyAnimations({
      neutral:"./static/animations/nanako_idle.fbx?v=11.9.9",
      angry:"./static/animations/nanako_angry.fbx?v=11.9.9",
      thinking:"./static/animations/nanako_thinking.fbx?v=11.9.9",
      clapping:"./static/animations/nanako_clapping.fbx?v=11.9.9"
    });
    ...
    console.log("[Nanako 3D] Step 1.99 body animations ready: neutral, angry, thinking, clapping");
    return true;
...
function playAnimationPlan(plan,{loop=false,onComplete=null,mediaClock=null}={}){
  ...
  const tick=now=>{
    ...
    let elapsed=mediaClock
      ...
      : now-started;
    // Step 1.99: every spoken plan follows the complete decoded media duration.
...
const CLIENT_BUILD="11.9.9",API="https://nanako-web-pokbkohedy.ap-southeast-1.fcapp.run",CHAT=`${API}/api/chat`,VISION_IDENTIFY=`${API}/api/vision/identify`,RESET=`${API}/api/reset`,STARTUP_GREETING=`${API}/api/startup-greeting`;
const verifiedBuildMarker=document.getElementById("buildMarker");if(verifiedBuildMarker)verifiedBuildMarker.textContent=`v11 Step 1.99 • Qwen3.5-Omni-Plus • JavaScript ${CLIENT_BUILD} verified`;
...
async function ensureMicHardware(){
  ...
  if(ctx.audioWorklet&&window.AudioWorkletNode){
    try{
      await ctx.audioWorklet.addModule("./static/mic-transport-worklet.js?v=11.9.9");
  ...
  try{
    ...
    if(cameraVoiceMode&&!e.cameraModal.hidden){payload.image_data_url=captureCameraFrame();payload.trigger="rear_object_camera";e.cameraStatus.textContent="Nanako is looking and listening..."}
      ...
      if(awarenessActive){
        ...
        payload.trigger="front_camera_request";payload.visual_target=String(inspection.visual_target||"face_or_scene");frontVisionAttached=true;console.log(`[Nanako Vision 11.9.9] one authorized front frame attached • target=${payload.visual_target} • chars=${payload.image_data_url.length}`);status("Nanako is looking...")
      }
...
async function boot(){
  ...
  console.log(`[NanaChat] v11 Step 1.99 QWEN3.5-OMNI-PLUS + FAST SPEECH-END RESPONSE VERIFIED runtime=${CLIENT_BUILD} • learner model: ${learnerMemory.preferences.length} preferences, ${learnerMemory.language_progress.length} language patterns • user=${persistentUserName||"unknown"}`);
}
