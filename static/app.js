...
function setVoiceOutput(v){
  voiceOutput=!!v;
  updateVoiceOutputUi();
  console.log(`[Nanako Omni] Voice output ${voiceOutput?"ON":"OFF"}`);
}
...
async function uploadVoice(b){if(!active)return;busy=true;status("Nanako is thinking...");try{let f=new FormData(),type=b.type||"audio/mp4",ext=type.includes("mp4")?"m4a":type.includes("ogg")?"ogg":"webm";f.append("audio",b,`nanako_voice.${ext}`);f.append("level",level);const requestedVoiceOutput = voiceOutput;
  f.append("voice_output", requestedVoiceOutput ? "true" : "false");
  console.log(`[Nanako Voice Request] voice_output=${requestedVoiceOutput} muted=${muted}`);let r=await fetch(VOICE,{method:"POST",body:f}),d=await jsonResp(r);if(d?.ignored){if(active){status("Listening...");setTimeout(begin,60)}return}let t=String(d?.transcript||"");console.log("[Nanako Web Voice] Transcript:",t);transcript(t);await apply(d,t)}catch(x){console.error(x);error(x.message);status("Voice turn failed");if(active)setTimeout(begin,500)}finally{busy=false}}
