...
async function ensureCurrentServiceWorker(){
  ...
  try{
    const reg=await navigator.serviceWorker.register("./sw.js?v=2.6.4",{scope:"./",updateViaCache:"none"});
    await reg.update();
...
async function loadOptionalBodyAnimations(){
  ...
  const result=await renderer.loadBodyAnimations({
    angry:"./static/animations/nanako_angry.fbx?v=2.6.4",
    thinking:"./static/animations/nanako_thinking.fbx?v=2.6.4",
    clapping:"./static/animations/nanako_clapping.fbx?v=2.6.4"
  });
...
async function loadProductionBodyAnimations(){
  ...
  bodyAnimationsLoading=(async()=>{
    const result=await window.nanako3DRenderer.loadBodyAnimations({
      standing:"./static/animations/nanako_idle_replacement.fbx?v=2.6.4"
    });
...
function preloadAnimationSfx(){
  void loadAnimationSfxBuffer("./static/sfx/wink.wav?v=2.6.4");
  void loadAnimationSfxBuffer("./static/sfx/single_clap.wav?v=2.6.4");
  void loadAnimationSfxBuffer("./static/sfx/level_up.mp3?v=2.6.4");
  void loadAnimationSfxBuffer("./static/sfx/daily_challenge_pass.wav?v=2.6.4");
}

async function playLevelUpSfx(){
  const ac=animationSfxContext();if(!ac)return false;const buffer=await loadAnimationSfxBuffer("./static/sfx/level_up.mp3?v=2.6.4");if(!buffer)return false;try{if(ac.state!=="running")await ac.resume()}catch{}
...
function levelUpClapPlan(){return {source:"nanako-learning-reward",renderer_contract:"nanako-vrm-1.1-python-plan-fbx",renderer_mode:"vrm-3d-only",kind:"level-up-clapping",emotion:"happy",body_motion:"clapping",duration_ms:2600,sample_ms:100,sfx:{src:"./static/sfx/single_clap.wav?v=2.6.4",hits_ms:[140,432,724,1016,1308,1600,1892,2184,2476],hit_volumes:[1.00,0.60,0.82,0.48,0.74,0.56,0.90,0.64,0.78],stop_ms:2600,name:"single_clap_levelup",volume:1},frames:[{t:0,emotion:"happy",action:"clapping",eyes:"open",mouth:"closed",body_motion:"clapping",scale:1,translate_y:0},{t:2500,emotion:"happy",action:"clapping",eyes:"open",mouth:"closed",body_motion:"clapping",scale:1,translate_y:0},{t:2600,emotion:"happy",action:"idle",eyes:"open",mouth:"closed",body_motion:"neutral",scale:1,translate_y:0}]};}
...
async function playDailyChallengePassSfx(){
  const ac=animationSfxContext();if(!ac)return false;const buffer=await loadAnimationSfxBuffer("./static/sfx/daily_challenge_pass.wav?v=2.6.4");if(!buffer)return false;
...
function playAnimationPlan(plan,{loop=false,onComplete=null,mediaClock=null}={}){
  ...
  if(plan.kind==="post-speech-wink"&&!plan.sfx){
    plan={...plan,duration_ms:Math.max(900,Number(plan.duration_ms)||0),sfx:{src:"./static/sfx/wink.wav?v=2.6.4",start_ms:400,stop_ms:1400,name:"wink-post-speech",volume:1}};
  }
...
const CLIENT_BUILD="12.0.139",RELEASE_VERSION="2.6.4",API="https://nanako-web-pokbkohedy.ap-southeast-1.fcapp.run",CHAT=`${API}/api/chat`,VISION_IDENTIFY=`${API}/api/vision/identify`,RESET=`${API}/api/reset`,STARTUP_GREETING=`${API}/api/startup-greeting`,REALTIME_ENRICH=`${API}/api/realtime/enrich`;
const NANAKO_OUTFIT_MODELS=Object.freeze({
  neutral:"./static/models/nanako_avatar.vrm?v=2.6.4",
  sports:"./static/models/nanako_sports.vrm?v=2.6.4",
  casual:"./static/models/nanako_casual_tshirt_shorts.vrm?v=2.6.4",
  swimwear:"./static/models/nanako_swimwear.vrm?v=2.6.4",
  teacher:"./static/models/nanako_teacher.vrm?v=2.6.4",
  kimono:"./static/models/nanako_kimono.vrm?v=2.6.4",
  elegant:"./static/models/nanako_elegant_black_sleeveless.vrm?v=2.6.4"
});
...
function applyRealtimePostState({emotion="neutral",bodyMotion="neutral",eyeGesture="none"}={}){
  ...
  if(wink){
    ...
    scheduleAnimationSfx({duration_ms:1560,sfx:{src:"./static/sfx/wink.wav?v=2.6.4",start_ms:560,stop_ms:1560,name:"wink-realtime",volume:1}});
  ...
  if(realtimeBodyMotion==="clapping"){
    stopAnimationPlan();scheduleAnimationSfx({duration_ms:2600,sfx:{src:"./static/sfx/single_clap.wav?v=2.6.4",hits_ms:[140,432,724,1016,1308,1600,1892,2184,2476],hit_volumes:[1.00,0.60,0.82,0.48,0.74,0.56,0.90,0.64,0.78],stop_ms:2600,name:"single-clap-realtime",volume:1}});renderAnimationFrame({emotion:realtimeEmotion||"happy",action:"clapping",eyes:"open",mouth:"closed",body_motion:"clapping",scale:1,translate_y:0});
...
async function begin(){
  if(!active)return;
  /* v2.6.4: endpoint locally, then upload one complete turn. Function Compute
...
// v2.6.4 — One reason-based music duck controller. Voice activity and Nanako playback
